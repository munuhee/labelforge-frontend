          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                <Link href={`${basePath}/tasks/${review.taskId}`} className="font-medium text-sm text-foreground hover:text-primary truncate">
                  {review.taskTitle}
                </Link>
          ...
          <Tabs defaultValue="all" className="space-y-3" onValueChange={() => setPage(1)}>
            ...
            <TabsContent value="all">
              <div className="space-y-2">
                {filtered.length ? pagedSlice(filtered).map(r => <ReviewCard key={r.id} review={r} />) : <EmptyState />}
                <PaginationBar page={page} total={filtered.length} pageSize={PAGE_SIZE} onPage={setPage} />
              </div>
            </TabsContent>
            <TabsContent value="pending">
              <div className="space-y-2">
                {pending.length ? pagedSlice(pending).map(r => <ReviewCard key={r.id} review={r} />) : <EmptyState />}
                <PaginationBar page={page} total={pending.length} pageSize={PAGE_SIZE} onPage={setPage} />
              </div>
            </TabsContent>
            <TabsContent value="approved">
              <div className="space-y-2">
                {approved.length ? pagedSlice(approved).map(r => <ReviewCard key={r.id} review={r} />) : <EmptyState />}
                <PaginationBar page={page} total={approved.length} pageSize={PAGE_SIZE} onPage={setPage} />
              </div>
            </TabsContent>
            <TabsContent value="action">
              <div className="space-y-2">
                {needsAction.length ? pagedSlice(needsAction).map(r => <ReviewCard key={r.id} review={r} inNeedsAction />) : <EmptyState />}
                <PaginationBar page={page} total={needsAction.length} pageSize={PAGE_SIZE} onPage={setPage} />
              </div>
            </TabsContent>
          </Tabs>
        )}
